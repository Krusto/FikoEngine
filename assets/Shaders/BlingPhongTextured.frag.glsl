...
void main()
{
    ...
    // diffuse color
    vec3 diffuse  = light.diffuse * (diff * texture2D(vertexData.texCoord,diffuseTexture));  
    
    // specular color
    vec3 specular =  light.specular * (spec * texture2D(vertexData.texCoord,specularTexture));  
