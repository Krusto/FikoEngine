...
namespace FikoEngine{
    std::vector<VkSemaphore> CreateSemaphores(RendererDataAPI*  rendererData, u32 count){
        ...
        for (int i = 0; i < count; ++i) {
            ...
            VK_CHECK(vkCreateSemaphore(rendererData->device,&createInfo,rendererData->allocator,&semaphores[i]));
    ...
    std::vector<VkFence> CreateFences(RendererDataAPI*  rendererData,u32 count){
        ...
        for (int i = 0; i < count; ++i) {
            ...
            VK_CHECK(vkCreateFence(rendererData->device,&createInfo,rendererData->allocator,&fences[i]));
